...
hover:to-secondary relative"
      onMouseEnter={() => setIsHovered(true)} // when Mouse hover on the icon it displays tool tip, for this we are using usestate
    ...
    >
      ...
      <AnimatePresence>
        {isHovered && (
          <motion.div
            initial={{ opacity: 0, x: -25 }} //animation for icon hover text from initial to exit
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: -25 }}
            className=" hidden lg:block absolute bg-white rounded-md px-4 py-1 -left-[140px]
